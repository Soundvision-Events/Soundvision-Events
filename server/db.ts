...
import { InsertUser, users, contactSubmissions, InsertContactSubmission, files, InsertFileRecord } from "../drizzle/schema";
...
export async function updateContactStatus(id: number, status: "new" | "in_progress" | "confirmed" | "completed" | "cancelled") {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
